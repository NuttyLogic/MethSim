...
def simulate_site(site_deviation=0.0, max_delta=1.0):
    ...
    if pos:
        target = m_not_i + max_delta
        target = 1.0 if target > 1.0 else target
        m_target = np.random.uniform(m_not_i, target)
    else:
        target = m_not_i - max_delta
        target = 0.0 if target < 0.0 else target
        m_target = np.random.uniform(target, m_not_i)
    return np.array([m_not_i, m_target, std])
...
def simulate_site_matrix(deviation_low=0.001, deviation_high=0.1,
    ...
    for site in range(number_of_sites):
        ...
        sites[site, :] = simulate_site(site_deviation=site_dev, max_delta=site_delta)
    return sites
